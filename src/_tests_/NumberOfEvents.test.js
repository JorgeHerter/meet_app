...
describe('<NumberOfEvents /> component', () => {
  ...
  test('ensures the value is between 1 and 50 after backspace and typing', () => {
    ...
    // Now simulate deleting and typing a new valid value within range
    fireEvent.change(inputField, { target: { value: '' } }); // Simulate backspace (clear the value)
    fireEvent.change(inputField, { target: { value: '10' } }); // Type a new number (valid)
    expect(inputField.value).toBe('10');
  });
});
